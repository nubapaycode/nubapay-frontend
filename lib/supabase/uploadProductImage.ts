...
import type { WorkspaceProduct } from '@/lib/organizerWorkspace'

const MAX_BYTES = 5 * 1024 * 1024

export async function uploadWorkspaceProductImage(
  eventId: string,
  productId: string,
  file: File,
  ...
  if (!token) {
    return { ok: false, error: 'Iniciá sesión para subir la imagen.' }
  }

  const form = new FormData()
  form.append('file', processed, processed.name || 'product.webp')

  const res = await browserFetch(eventsPaths.workspace(eventId, `products/${productId}/image`), {
    method: 'POST',
...
export async function clearWorkspaceProductImage(
  eventId: string,
  productId: string,
): Promise<{ ok: true; product: WorkspaceProduct } | { ok: false; error: string }> {
  const token = getAuthToken()
  if (!token) {
    return { ok: false, error: 'Iniciá sesión para quitar la imagen.' }
  }
  ...
  const body = (await res.json()) as { product?: WorkspaceProduct; error?: string }
  if (!res.ok || !body.product) {
    return { ok: false, error: body.error ?? 'No se pudo quitar la imagen' }
  }
