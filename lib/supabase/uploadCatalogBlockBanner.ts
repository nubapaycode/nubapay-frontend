...
import type { WorkspaceCatalogBlock } from '@/lib/organizerWorkspace'

const MAX_BYTES = 5 * 1024 * 1024

export async function uploadCatalogBlockBanner(
  eventId: string,
  blockId: string,
  file: File,
  ...
  form.append('file', processed, processed.name || 'banner.webp')

  const res = await browserFetch(eventsPaths.workspace(eventId, `blocks/${blockId}/banner`), {
    method: 'POST',
...
export async function clearCatalogBlockBanner(
  eventId: string,
  blockId: string,
): Promise<{ ok: true; block: WorkspaceCatalogBlock } | { ok: false; error: string }> {
  const token = getAuthToken()
