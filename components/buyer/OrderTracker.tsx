...
export function OrderTracker({ orderId, catalogSlug }: OrderTrackerProps) {
  ...
  // El CTA de pago pendiente debe reflejar el método real de la orden, no
  // asumir siempre Mercado Pago (ahora también puede ser Sipago).
  const methodMeta: Record<string, { label: string; color: string; bg: string }> = {
    mp: { label: 'Mercado Pago', color: '#009EE3', bg: 'rgba(0,158,227,0.18)' },
    sipago: { label: 'Sipago', color: '#4E358B', bg: 'rgba(78,53,139,0.18)' },
  }
  const activeMethod = order?.payment_method ? methodMeta[order.payment_method] : undefined
  const paymentMethodLabel = activeMethod?.label ?? 'tu medio de pago'
        ...
        <div className="mx-auto flex w-full max-w-lg flex-1 flex-col items-center justify-center gap-6 px-4 py-10">
          ...
          >
            ...
            <div className="flex flex-col items-center gap-1 text-center">
              ...
              <p className="text-[12px]" style={{ color: BUYER_COLORS.muted }}>Te vamos a redirigir a {paymentMethodLabel} en un momento.</p>
            </div>
      ...
      <div className="mx-auto flex w-full max-w-lg flex-1 flex-col gap-3 px-4 py-5" style={{ display: redirecting ? 'none' : undefined }}>
        ...
        {!isPaid && paymentResult === 'pending' && (
          <div className="flex items-center gap-3 rounded-[16px] px-4 py-3" style={{ background: '#FFFBEB', border: '1px solid #FDE68A' }}>
            ...
            <p className="text-[13px] font-semibold" style={{ color: '#92400E' }}>Tu pago está siendo procesado por {paymentMethodLabel}.</p>
          </div>
        )}
          ...
          >
            ...
            <div className="px-4 pb-4 flex flex-col gap-3">
              <button
                ...
                style={{
                  background: activeMethod?.bg ?? BUYER_COLORS.subtleFill,
                  color: activeMethod?.color ?? BUYER_COLORS.text,
                }}
              >
                {mpCheckoutReady ? (
                  <>
